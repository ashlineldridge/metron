...
pub fn parse<I, T>(it: I) -> Result<CommandSpec, Error>
where
    I: IntoIterator<Item = T>,
    T: Into<OsString> + Clone,
{
    if let Err(e) = root::command().try_get_matches_from(it) {
        match e.kind() {
            ErrorKind::DisplayHelp | ErrorKind::DisplayVersion => {
                return Ok(CommandSpec::Help(e.render().to_string()));
            }
            _ => (),
        }
    }
    ...
    //             return Ok(CommandSpec::Help(e.render()));
    //         }
...
// // TODO: Does all of this need to be pub?
// #[derive(Clone, Debug)]
// pub enum Config {
//     // Agent(crate::agent::)
