...
** Code Comments
*** Function Comments
Use third person =// Creates the foo= rather than second person =// Create the foo=. This reads more naturally and seems to be in-keeping with the Rust standard library (e.g. =Option=). See also the Google Style Guide [[https://developers.google.com/style/reference-verbs][here]].
